...
<section class="rt_wrap content mCustomScrollbar">
    <div class="rt_content">
        <div class="page_title">
            <h2 class="fl">主题列表</h2>
            <a href="http://localhost:80/manager/theme/addView" class="fr top_rt_btn add_icon">添加主题</a>
        </div>
...
<script>
    function init(){
        $.ajax({
            url:"http://localhost:80/theme/list",
            async: false,
            type:"get",
            dataType:"json",
                        ...
                        $.ajax({
                            ...
                            success:function(result){
                                ...
                                $.each(data.records,function(i,item){
                                    var edtior="<td class='center'><a href='http://localhost:80/manager/theme/detailView/"+item.id+"' title='编辑' class='link_icon' >&#101;</a><a  title='删除' class='link_icon'value='"+item.id+"' onclick='del(this)'>&#100;</a></td>";
                                    showTableHtml += "<tr>"+
    ...
    function del(obj) {
        ...
        if(confirm("确实要删除吗？")){
            $.ajax({
                type:"GET",
                url:"http://localhost:80/manager/theme/delete/"+id,
                dataType:"json",
                success:function(obj){
                    if(obj.status==200){
                        alert("删除成功");
                        window.location.href="http://localhost:80/admin/themelist";
                    }
