...
<script>
    function del(obj) {
        ...
        if(confirm("确实要删除吗？")){
            $.ajax({
                type:"POST",
                url:"http://localhost:80/manager/user/delete",
                data:{
                    id:id
                },
                dataType:"json",
...
<script>
    ...
    function init(){
        $.ajax({
            url:"http://localhost:80/manager/user/count",
            async: false,
            type:"get",
            dataType:"json",
            success:function(data){
                ...
                $.jqPaginator('#pagination', {
                    ...
                    onPageChange: function (pageIndex) {
                        ...
                        $.ajax({
                            url:"http://localhost:80/manager/user/list",
                            async: false,
                            data:{
                                current:pageIndex,
                                size:5
                            ...
                            success:function(result){
                                ...
                                $.each(data.records,function(i,item){
                                    var edtior="<td class='center'><a href='http://localhost:80/manager/user/updateView/"+item.id+"' title='编辑' class='link_icon'>&#101;</a><a  title='删除' class='link_icon'value='"+item.id+"' onclick='del(this)'>&#100;</a> </td>";
                                    showTableHtml += "<tr>"+
